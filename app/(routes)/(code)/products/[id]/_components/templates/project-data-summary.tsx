...
export default async function ProjectDataSummary({
  productId,
}: {
  productId: string;
}) {
  ...
  return (
    ...
    <ul className="bg-background text-foreground flex flex-col gap-4">
      ...
      <LiItem>
        ...
        <span className="flex items-center gap-2">
          ...
          <span className="font-medium">
            {topSentiment} | {basicAnalytics.sentiment.percent}%
          </span>
        </span>
      </LiItem>
      <Separator />
      <LiItem>
        ...
        <span className="font-medium">
          {basicAnalytics.topic.topTopic} |{" "}
          {basicAnalytics.topic.topTopicPercent}%
        </span>
      </LiItem>
    </ul>
  );
