...
export default function FeedbackItem ({ feedbackData, productId }) {
    ...
    return (
      <div className="bg-background p-4 rounded-md space-y-3 border border-border">
        ...
        {/* Idea Content */}
        <div>
          <h3 className="font-semibold text-foreground mb-2 max-w-full">{idea.title}</h3>
          <p className="text-foreground wrap flex-wrap" style={{wordBreak: `break-word`}}>
            {!idea.isRich && (isExpanded ? idea.text : `${idea.text.slice(0, 200)}...`)}
            {idea.isRich && <RichTextViewer className="text-secondaryForeground" content={idea.text}/>}
          </p>
        ...
        {/* Comments Section */}
      <CommentsSection feedbackId={feedbackData.feedbackId} productId={productId}/>
      </div>
    );
  };
