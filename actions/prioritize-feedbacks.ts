...
async function callDeepSeekAPI(
  ...
  const apiUrl =
    "https://api-inference.huggingface.co/models/mistralai/Mistral-7B-Instruct-v0.1t";
  const apiKey = process.env.TASK_PRIORITIZATION_HUGGINGFACE_API_KEY;
  const maxRetries = 5;

  if (!apiKey) {
    throw new Error(
      "Hugging Face API Key (HUGGINGFACE_API_KEY) is not configured in environment variables."
    );
  }
  ...
  try {
    ...
    if (!generatedText) {
      console.warn(
        "DeepSeek API response did not contain expected generated text.",
        result
      );
    ...
    throw new Error(
      `Failed to get response from DeepSeek API after ${
        maxRetries + 1
  ...
  .join(
    "\n"
        ...
        try {
          ...
          console.error(
            `Failed to parse JSON response for ${product.data.name}:`,
            parseError,
            "Raw response:",
            jsonStr
          );
